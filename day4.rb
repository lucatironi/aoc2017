...
def valid_pass?(phrase, strict = false)
  if strict
    h = phrase.split(" ").reduce(Hash.new(0)) do |hash, word|
      hash[word.chars.sort] += 1
      hash
    end.values.none? { |v| v > 1 }
  else
    phrase.split(" ").reduce(Hash.new(0)) do |hash, word|
      hash[word] += 1
      hash
    end.values.none? { |v| v > 1 }
  end
...
def check_pass_list(list, strict = false)
  a = list.each_line.map do |line|
    valid_pass?(line, strict)
  end
  a.reduce(0) { |count, valid| count += 1 if valid; count}
end

class TestDay4 < Minitest::Test
  def test_valid_pass?
    assert valid_pass?("aa bb cc dd ee")
    refute valid_pass?("aa bb cc dd ee aa")
    assert valid_pass?("aa bb cc dd ee aaa")
  end

  def test_valid_pass_strict?
    assert valid_pass?("abcde fghij", true)
    refute valid_pass?("abcde xyz ecdab", true)
    assert valid_pass?("a ab abc abd abf abj", true)
    assert valid_pass?("iiii oiii ooii oooi oooo", true)
    refute valid_pass?("oiii ioii iioi iiio", true)
  end

  def test_final
    assert_equal 466, check_pass_list(INPUT)
  end

  def test_final_strict
    assert_equal 251, check_pass_list(INPUT, true)
  end
end
