...
class Tower
  attr_reader :programs, :root

  def initialize(input)
    @programs = input.each_line.map { |line| Program.new(line) }

    @programs.each do |program|
      next if program.list.empty?

      program.list.each do |child_name|
        @programs.each do |node|
          if child_name == node.name
            node.parent = program if node.parent.nil?
    ...
    @root = @programs.find { |p| p.parent.nil? }
  end

  def find_by_name(name)
    @programs.select { |p| p.name == name }.first
  end
...
class Program
  attr_reader :name, :weight, :list
  attr_accessor :parent, :children

  def initialize(string)
    match = string.match(/(?<name>\w+)\s\((?<weight>\d+)\)(\s->\s(?<list>.*))?/)

    @name   = match[:name]
    @weight = match[:weight].to_i
    @list   = match[:list] ? match[:list].split(", ") : []
  ...
  def total_weight
    children.reduce(weight) { |sum, child| sum += child.total_weight; sum }
  end

  def print(counter)
    p "#{'--' * counter} #{name} #{weight} / #{total_weight}"
    children.each { |child| child.print(counter + 1) }
  end
end

class TestDay7 < Minitest::Test
  def test_program_init
    program = Program.new("a (1) -> b, c")
    assert_equal "a", program.name
    assert_equal 1, program.weight
    assert_equal ["b", "c"], program.list
  end

  def test_program_init_without_list
    program = Program.new("a (1)")
    assert_equal "a", program.name
    assert_equal 1, program.weight
    assert_equal [], program.list
  end

  def test_tower_init
    tower = Tower.new(TRIVIAL_INPUT)
    a = tower.find_by_name("a")
    b = tower.find_by_name("b")
    c = tower.find_by_name("c")
    assert_equal 3, tower.programs.size
    assert_equal a, tower.root

    assert_nil a.parent
    assert_equal a, b.parent
    assert_equal a, c.parent

    assert_equal [b, c], a.children
  ...
  def test_find_root
    ...
    assert_equal "tknk", tower.root.name
  end

  def test_find_root_final
    tower = Tower.new(FINAL_INPUT)
    assert_equal "aapssr", tower.root.name
  end

  def test_total_weight_trivial
    tower = Tower.new(TRIVIAL_INPUT)
    a = tower.find_by_name("a")
    assert_equal 9, a.total_weight
  end

  def test_total_weight
    tower = Tower.new(INPUT)
    ugml = tower.find_by_name("ugml")
    padx = tower.find_by_name("padx")
    fwft = tower.find_by_name("fwft")
    assert_equal 251, ugml.total_weight
    assert_equal 243, padx.total_weight
    assert_equal 243, fwft.total_weight
  end
end
