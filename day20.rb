class Particle
  attr_reader :label, :px, :py, :pz, :vx, :vy, :vz, :ax, :ay, :az

  def initialize(label, px = 0, py = 0, pz = 0, vx = 0, vy = 0, vz = 0, ax = 0, ay = 0, az = 0)
    @label = label
    @px = px
    @py = py
    @pz = pz
...
class Simulation
  ...
  def initialize(input)
    ...
    input.each_line.with_index do |line, index|
      @particles << Particle.new(index, *line.scan(/-?\d+/).map(&:to_i))
    end
  end
  ...
  def update
    ...
    @particles.each do |particle|
      collisions = (@particles - [particle]).select do |other|
        particle.position == other.position
      end
      unless collisions.empty?
        @particles -= collisions.push(particle)
      end
    end
    p @particles.size
  end
  ...
  def run(steps)
    steps.times { update }
    p @particles.sort_by(&:distance).map{ |par| { label: par.label, distance: par.distance } }.take(5)
  end
end
